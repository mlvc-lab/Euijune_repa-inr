...
case $DSET in
    ...
    "Urban_100")
        ...
        ;;
    "KAGGLE")
        DATADIR="/local_dataset/KAGGLE"
        OUTCHANNEL=3
        SIDELEN=768/
        MAXITER=10
        ;;
    *)
        echo "Unknown dataset type. Please choose one of: Chest_CT, CelebA_HQ, DIV2K, TESTIMAGE"
        exit 1
        ;;
esac
...
# EXPNAME="$DSET/scratch/Finer/sidelen_$SIDELEN"
...
elem=('07')                                         # TESTIMAGES (1200 x 1200)
#for SOURCEIMGID in `seq -w 1 40`
for SOURCEIMGID in "${elem[@]}"
do
    EXPNAME="$DSET/$SOURCEIMGID/Finer"
    LOADPATH="logs/DIV2K/scratch/Finer/sidelen_$SIDELEN/outputs_$SOURCEIMGID.pt"
    unset imgid_map
    declare -A imgid_map  # 재선언하여 초기화


    #for TARGETIMGID in "${elem[@]}"
